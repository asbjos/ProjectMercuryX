...
#include "MercuryOnlyCapsule.h"
#include "..\samples\Experiments\ShuttlePBforReal\ShuttlePBforReal\MercuryCapsule.h"
...
int ProjectMercury::clbkConsumeBufferedKey(DWORD key, bool down, char* kstate)
{
	...
	else // single buttons are pressed (no Ctrl, Shift, Alt)
	{
		...
		case OAPI_KEY_C: // switch to launch complex
			if ((VesselStatus == LAUNCH || VesselStatus == TOWERSEP) && GetAttachmentStatus(padAttach) == NULL)
			{
...
bool ProjectMercury::clbkDrawHUD(int mode, const HUDPAINTSPEC* hps, oapi::Sketchpad* skp)
{
	...
	if (showInfoOnHud == 1) // Show Mercury dashboard left row of status lights. This is in the end to not affect right side font colour
	{
		...
		if ((retroStartTime != 0.0 && retroStartTime - 10.0 < simt && (!retroCoverSeparated[2] || FailureMode == RETROSTUCKOFF)) || (!engageRetro && (retroCoverSeparated[0] || retroCoverSeparated[1] || retroCoverSeparated[2]))) skp->SetTextColor(Red);
		...
		// .05 G
		if (AutopilotStatus == LOWG || ((VesselStatus == FLIGHT || VesselStatus == REENTRY) && vesselAcceleration > 0.05 * G)) skp->SetTextColor(Green);
		else skp->SetTextColor(Gray);
		sprintf(cbuf, ".05 G");
		skp->Text(TextX0, yIndex * LineSpacing + TextY0, cbuf, strlen(cbuf));
		yIndex += 1;
